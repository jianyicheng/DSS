...
string entity_name[] = {
    ...
    ENTITY_LSQ,     ENTITY_MC,          ENTITY_DISTRIBUTOR, ENTITY_SELECTOR};
...
string component_types[] = {
    ...
    COMPONENT_LSQ,       COMPONENT_MC,           COMPONENT_DISTRIBUTOR,
    COMPONENT_SELECTOR};
    ...
    if ((nodes[i].name.empty())) // Check if the name is not empty
    {
      ...
      for (indx = 0; indx < nodes[i].outputs.size; indx++) {
        ...
        for (int out_port_indx = 0;
          ...
          signal += to_string((nodes[i].outputs.output[indx].bit_size - 1 >= 0)
                                  ? nodes[i].outputs.output[indx].bit_size - 1
                                  : DEFAULT_BITWIDTH - 1);
          signal += " downto 0);";
          signal += '\n';
...
void write_entity(string filename, int indx) {
  ...
  if (indx == 0) // Top-level module
  {
    ...
    for (int i = 0; i < components_in_netlist; i++) {
      ...
      // if ( nodes[i].type.find("MC") != std::string::npos )
      {
        ...
        //                     "_data_from_memory : in std_logic_vector (31
        //                     downto 0);" << endl; netlist << "\t" <<
        //                     nodes[i].memory << "_read_enable : out
        //                     std_logic;" << endl; netlist << "\t" <<
        //                     nodes[i].memory << "_read_address : out
        //                     std_logic_vector (31 downto 0)"; netlist << ";"
        //                     << endl; netlist << "\t" << nodes[i].memory <<
        //                     "_data_to_memory : out std_logic_vector (31
        //                     downto 0);" << endl; netlist << "\t" <<
        //                     nodes[i].memory << "_write_enable : out
        //                     std_logic;" << endl; netlist << "\t" <<
        //                     nodes[i].memory << "_write_address : out
  ...
  } else // Sub-Module module
  {
    ...
    //         dataInArray : in data_array (1 downto 0)(DATA_SIZE_IN-1 downto
    //         0); dataOutArray : out data_array (0 downto 0)(DATA_SIZE_OUT-1
    //         downto 0); pValidArray : in std_logic_vector(1 downto 0);
